...
const initOneSentence = async () => {
  fetch("https://v1.hitokoto.cn/?c=d&encode=json")
    .then((res) => res.json())
    .then((res) => {
      saying.value = [`${res.hitokoto} —— ${res.from}`];
    })
    .catch((err) => {
      console.error("获取文艺句子失败：", err);
      saying.value = ["你来人间一趟，你要看看太阳 —— 海子"];
    });
};
...
const getBgCover = computed(() => {
  ...
  // 做一个根据路由来判断判断页面背景图片
  let url;
  let myUrl = "http://img.fbeer.cn/login-l.png";
